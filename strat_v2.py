...
pe_raw = pd.read_csv(f"{DATA_DIR}pe_ratio.csv",  index_col=0, parse_dates=True)
...
def backtest(factor_pct_dict, pct_thresholds, universe_mask=universe,
    ...
    # 持倉比例係數 = 該標的score / total score
    # score = a(1-eps_pct) + b(1-mom_pct) + c(low_pe_pct)
    # assume a,b,c = 1, tunable.
    holding_score = pd.DataFrame(0, index=selected_daily.index, columns=selected_daily.columns)
    for name, pct_df in factor_pct_dict.items():
        thr = pct_thresholds[name]
        score = (1 - pct_df) * (pct_df <= thr)
        holding_score += score
...
pe_clean = pe_raw.where((pe_raw > 3) & (pe_raw <= 700), np.nan)
...
factor_pcts = {
    ...
    'Low_PE':      pct_rank_in_universe(pe_clean,    lower_better=True),
}
...
output_path = "strategy_data/returns/StarSearch_v2.json"
...
with open("strategy_data/holdings/StarSearch_v2.json", "w", encoding="utf-8") as f:
    json.dump(result, f, ensure_ascii=False, indent=2)